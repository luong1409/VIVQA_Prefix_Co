...
class PrefixDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print('prefix_text_len', prefix_text_len)
        image = self._get_transformed_image(str(img_cap_pair['image']))
        # tokenize
        assert self.processor is not None
        prefixs, tgts = self._get_tokenized_text(caption[0], prefix_text_len)
        return image, prefixs, tgts, caption
    ...
    def _get_tokenized_text(self, text, prefix_text_len):
        ...
        tokenized = tokenizer.tokenize(text) # PhoBert.tokenize --> sentence to token
